...
namespace objl
{
	...
	private:
        ...
        void GeometryObject()
        {
            ...
            const int s1 = loader.LoadedVertices.size();
            VertexData *vertices = new VertexData[s1];

            for (int i =0;i < loader.LoadedVertices.size(); i ++)
            {
                objl::Vertex p = loader.LoadedVertices[0];
                QVector3D P(p.Position.X, p.Position.Y, p.Position.Z);
                QVector2D T(p.TextureCoordinate.X, p.TextureCoordinate.Y);
                VertexData V;
                V.position = P;
                V.texCoord = T;
                vertices[i] = V;
            }
            const int s2 = loader.LoadedIndices.size();
            GLushort *indices = new GLushort[s2];
            ...
            arrayBuf.bind();
            arrayBuf.allocate(vertices, loader.LoadedVertices.size() * sizeof(VertexData));

            // Transfer index data to VBO 1
            indexBuf.bind();
            indexBuf.allocate(indices, loader.LoadedIndices.size() * sizeof(GLushort));
        }
    };
}
